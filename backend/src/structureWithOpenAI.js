...
You ONLY use facts that appear in the provided scrape. Do not invent phone numbers, prices, guarantees, or locations.
If something is unclear or missing, use null or empty arrays.
...
- contact: { "phones": string[], "emails": string[], "addresses": string[], "otherLinks": string[] }
...
- topicsCustomerMightAsk: string[] (likely user questions, grounded in text)
...
export async function structureWebsiteForChatbot(input) {
  ...
  const ownerBlock =
    input.owner && (input.owner.name || input.owner.email || input.owner.phone)
      ? `\n\nBusiness owner contact (from form, not from website): name=${input.owner.name || ''}, email=${input.owner.email || ''}, phone=${input.owner.phone || ''}`
      : ''
